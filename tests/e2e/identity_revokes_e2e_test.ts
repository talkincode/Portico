...
/**
 * Identity-revoke history is one contract: CLI `identity revokes`, Portal
 * `GET /api/revokes` and MCP `portico_revokes` must return the same
 ...
 * rewrite the roster or leak tokens or hashes. This is not a revoke write
 * path.
 */
...
async function mcpRevokes(
  url: string,
  session: string | null,
): Promise<Envelope<RevokeRow[]>> {
  ...
  return envelope<RevokeRow[]>(await response.json() as JsonRpcBody);
}

async function portalRevokes(
  url: string,
  session: string | null,
): Promise<{ status: number; body: Envelope<RevokeRow[]> }> {
  const response = await fetch(`${url}/api/revokes`, {
    headers: authHeaders(session),
  });
  return {
    status: response.status,
    body: await response.json() as Envelope<RevokeRow[]>,
  };
}

async function cliRevokes(
  identities: string,
  extra: string[] = [],
): Promise<{ code: number; body: Envelope<RevokeRow[]> }> {
  ...
  return {
    code: result.code,
    body: result.stdout as Envelope<RevokeRow[]>,
  };
}

function subjectsOf(rows: RevokeRow[] | undefined): string[] {
  return (rows ?? []).map((item) => item.subjectId);
}

Deno.test("E2E: CLI, Portal and MCP revoke trails match for auditor; others are refused", async () => {
  const dir = await Deno.makeTempDir({ prefix: "portico-revokes-e2e-" });
  ...
  try {
    ...
    const cliAuditor = await cliRevokes(identities, auditorAuth);
    const portalAuditor = await portalRevokes(portalUrl, auditorSession);
    const mcpAuditor = await mcpRevokes(mcpUrl, auditorSession);
    ...
    const cliMaintainer = await cliRevokes(identities, maintainerAuth);
    const portalMaintainer = await portalRevokes(portalUrl, maintainerSession);
    const mcpMaintainer = await mcpRevokes(mcpUrl, maintainerSession);
    ...
    const cliReader = await cliRevokes(identities, readerAuth);
    const portalReader = await portalRevokes(portalUrl, readerSession);
    const mcpReader = await mcpRevokes(mcpUrl, readerSession);
    ...
    const cliAnon = await cliRevokes(identities);
    const portalAnon = await portalRevokes(portalUrl, null);
    const mcpAnon = await mcpRevokes(mcpUrl, null);
    ...
    const posted = await fetch(`${portalUrl}/api/revokes`, {
      method: "POST",
      headers: authHeaders(auditorSession),
    });
    assertEquals(posted.status, 405);
    const postedBody = await posted.json() as Envelope<RevokeRow[]>;
    assertEquals(postedBody.error?.code, "USAGE");

    assertEquals(
      await Deno.readFile(identities),
      beforeIdentities,
      "reading revokes must not rewrite identities.json",
    );
    assertEquals(
      await Deno.readFile(sessions),
      beforeSessions,
      "reading revokes must not rewrite sessions.json",
    );
    const afterCatalog = await Deno.readFile(catalog).catch(() => new Uint8Array());
    assertEquals(
      afterCatalog,
      beforeCatalog,
      "reading revokes must not rewrite the catalog",
    );
