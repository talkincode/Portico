...
#
# The supervisor (src/up/main.ts) starts Portal :8788, Gateway :8789 and
# MCP :8790 as separate processes with their own permission sets from
# src/perms.ts (Portal/MCP read-only, Gateway write-scoped to its audit
# file). If any entrance exits, the supervisor stops the rest, so there is
