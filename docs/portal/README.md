...
| **`/internal/pending`** | 组织内部人员 | 持有会话 (Reader+) | 待审公开队列：当前身份可见的 `pending_public` 候选。可按 `?channel=`（cli / mcp / web）只读筛选。入口标明种类（url / package / mcp_endpoint），引用为转义文本、不可点击。无脚本、无表单，不能批准或驳回。匿名 HTML 404。 |
