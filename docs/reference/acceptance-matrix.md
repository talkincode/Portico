...
| **Review 独立人类审核入口** | 高 | `tests/review_handler_test.ts`；`tests/portal_review_entry_test.ts`；`tests/e2e/review_http_e2e_test.ts`；`tests/e2e/system_up_e2e_test.ts` |
...
| **安全审计结论（维护权 ≠ 审计权）** | 高 | `tests/audit_conclusions_test.ts`；`tests/e2e/audit_conclusions_e2e_test.ts`；`tests/e2e/audit_standings_e2e_test.ts` |
...
| **审计时间切片读取（截至某时刻重建）** | 高 | `tests/audit_instant_test.ts`；`tests/audit_query_test.ts`；`tests/audit_conclusions_test.ts`；`tests/e2e/audit_timeslice_e2e_test.ts` |
| **审计读取窗口声明（封条不切片，宁拒不猜）** | 中 | `tests/audit_window_test.ts`；`tests/portal_ui_test.ts`；`tests/e2e/audit_window_scope_e2e_test.ts` |
