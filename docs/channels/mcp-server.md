...
Portico MCP 服务端暴露了 24 个经过安全收敛的只读治理工具：
...
| **`portico_audit_verify`** | `{ asOf?: string }` | **仅限人类审计者** | 重算安全审计的封条链，报告每个环节是否与写入时一致：哪条记录被改写、被删除或链条断开，也会列出没有任何环节覆盖的记录（未封存）。等价于 CLI `audit verify` 与 Portal `GET /api/audit-verify`。载荷的 `window` 恒为 `"current"`：封条重算的是此刻的文件，没有历史模式，传 `asOf`（或 CLI `--as-of`、Portal `?asOf=`）得到 `INVALID_INPUT` 而不是被静默忽略。非审计者返回 `FORBIDDEN`，且角色检查先于该拒绝。只读：不写审计文件，也不修改任何记录；返回的 `tip` 是该链当前末端摘要，可与外部留存的摘要比对。 |
...
| **`portico_trash`** | `{}` | **维护者与人类审计者** | 列出回收站里的软删除记录（含删除人、删除时间与删除前状态）。与 CLI `catalog trash` 同一载荷。只读与匿名返回 `FORBIDDEN`。恢复走 CLI `catalog restore`，永久删除走 CLI `catalog purge`（仅人类审计者）。 |
