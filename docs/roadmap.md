...
`src/portal/` 是只读 HTTP 入口，消费同一 `CatalogService`。`GET /api/catalog`、`GET /api/catalog/:id`、`GET /api/dashboard`、`GET /api/mcp`、`GET /api/mcp/:id`、`GET /api/audit` 与 `GET /`（HTML）对同一身份呈现与 CLI 相同的可见性（审计面仅人类审计者）。无 actor 头为匿名；`X-Portico-Actor-*` 对照身份名册解析，不能自封角色。默认绑定 `127.0.0.1`。POST/PUT/PATCH/DELETE 返回 405，不写目录。登录会话尚未实现。
