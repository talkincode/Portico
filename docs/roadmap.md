...
| 部署契约与入口健康门禁 | 中 | ✅ 只读门禁先断言验证的是哪个地址（`PORTICO_DEPLOY_BIND` 声明，未声明时取监听表里唯一的那个地址；同端口有两个就 FAIL 而不猜），且三个入口都不绑在所有接口上；再打三个真实入口：发现页（按发现壳结构判产品页，不只看 200）、`/public` 200、`/internal` 404、目录 `ok:true` 信封、Portal 写方法 405、Gateway 工具调用 POST 405、Gateway 审计匿名 403、MCP `initialize` 返回 `portico`、MCP GET 405、监听进程不比它服务的树更旧；页眉只挂出本部署声明服务的审核入口——声明 `off`（三入口测试部署的默认）时产品页不得出现 `/review` 链接，声明 origin 时链到该 origin | ✅ 入口不在声明的地址上（点名声明的地址与真正应答的地址）、端口无人监听（点名端口）、同一端口两个监听地址（拒绝猜测；声明其中一个也仍 FAIL 并点名另一个）、入口绑在所有接口上、入口停机、错误页冒充产品页（同 `<title>` 同壳）、匿名 `/internal` 返 200、监听进程早于树的写入、`PORTICO_EXPECT_SHA` 与检出不符、产品页挂出本部署不提供的审核入口（声明 `off` 却出现 `/review`，或声明了 origin 却不链过去）——每条按名字报 `FAIL` 且非零退出 | ✅ 不适用：门禁只用匿名只读探测，不需要身份也不需要 sudo；Portal 与 MCP 仍不持 `--allow-write`，门禁不碰名册 | ✅ 门禁自身无副作用：失败只报名字与非零退出，不重启、不改目录、不留残文件（临时戳只在 `$TMPDIR`）；`skip` 只在不适用时出现，且不冒充通过 | `tests/e2e/deploy_verify_e2e_test.ts`；`tests/deploy_contract_test.ts` |

| macOS 生产部署门禁与运行修订 | 中 | ✅ 只读门禁在宿主机上回答三件事：检出修订（给了 `PORTICO_EXPECT_SHA` 才报该行）、四个入口（Portal 8788 / Gateway 8789 / MCP 8790 / Review 8791）**各自由哪个进程服务**、每个入口各自兑现契约——Portal `/public` 200、Review `/review/login` 200、公网 origin 上两条对应探测、匿名目录 `ok:true` 信封、匿名 `/internal` 404、Gateway 工具调用 POST 405、MCP `initialize` 返回 `portico`；默认值就是本机部署形状（回环 + 一个公网 origin），四个端口与 origin 都可由环境覆盖，因此门禁本身可以被指向测试自己的监听者并跑出全绿 | ✅ 入口没有进程在服务（点名入口与端口，且是 FAIL 而非 `skip`）、监听进程比 `PORTICO_DEPLOY_TREE/src` 里的代码旧（点名进程与它先于哪个文件写出）、`PORTICO_EXPECT_SHA` 与检出不符、Gateway 真的执行了工具、MCP 返回 200 但不是 portico 的 initialize 结果、检出里没有 `src` 可比、`mktemp` 不可用——每条按名字报 `FAIL` 且非零退出 | ✅ 不适用：门禁只用匿名只读探测，不需要 sudo，不读名册也不改目录状态 | ✅ 门禁自身无副作用：失败只报名字与非零退出，不重启、不改目录、不覆盖 `portico.env`；比对时间用的锚点只在 `$TMPDIR` 且用完即删 | `tests/e2e/deploy_macos_verify_e2e_test.ts`；`tests/deploy_macos_contract_test.ts` |
