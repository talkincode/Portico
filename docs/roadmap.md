...
本节的形状由**仓库合同检查**执行，不靠人工巡检：`deno task check:runtime-boundary`（`tests/runtime_boundary.ts` + `tests/runtime_boundary_test.ts`，并随 `deno task test` 在 CI 执行）扫描 `git ls-files` 的 tracked 树，拒绝 Node/Bun 的依赖根（`package.json`、`package-lock.json`、`yarn.lock`、`pnpm-lock.yaml`、`bun.lock`、`.nvmrc` 等，任意目录深度）、vendored `node_modules`、CI 里安装 Node/Bun 或调用其包管理器（`actions/setup-node`、`oven-sh/setup-bun`、`run: npm ci` / `npx` / `pnpm` / `bun install`）、`src/` 里的 `node:` 导入与对 `node`/`npm`/`npx`/`bun` 的子进程调用，以及任务、脚本与 CI 里的全开或未限定授权（`--allow-all`、`-A`、裸 `--allow-net`、裸 `--allow-ffi`）。

两处**刻意不判失败**，因为它们不是本仓库给自己开的权限：`npm:` 导入是 L0 允许的适配层；裸 `--allow-read` / `--allow-write` / `--allow-env` / `--allow-run` 是显式白名单，把它们收窄到具体路径或程序是部署合同的职责（由 `tests/deploy_contract_test.ts` 针对 `src/perms.ts` 与 `deploy/run-*.sh` 执行）。门户页里渲染给读者的第三方包运行命令同理——Portico 不执行、不授权，那条规则只管本仓库真正施加的授权。内容范围限定在会 ship 或会跑的 `src/`、`deno.json`、`deploy/`、`.github/`；`tests/` 只受清单路径规则约束，因为夹具必须能写出被拒的形状。失败输出只给 `文件:行:规则:命中内容`，tracked 树读不出来（`git` 不可用）时 fail closed，不静默通过。
...
本仓库开源，tracked 文件即公开面。`tests/redaction.ts` + `tests/public_surface_redaction_test.ts`（`deno task check:redaction`，并随 `deno task test` 在 CI 执行）扫描 `git ls-files` 的每个文件，拒绝两类值：凭据（与 catalog 校验同一批 issuer 前缀族与固定形状族：AWS Access Key ID、Google API Key、PEM 私钥头、npm 访问令牌、GitHub / GitLab / Slack / OpenAI 形值）与**线上内网坐标**（RFC1918、链路本地 `169.254/16`、CGNAT `100.64/10`、IPv6 ULA 与链路本地字面量、以及公网 DNS 不可能有的单标签主机 URL）。规则刻意按形状而不按名单：把线上主机名逐个写进检查等于把它们写进公开仓库，与 `approve` 的内网坐标检查同一理由。回环地址不在内（它是文档里的合法占位符），`0.0.0.0` 也不在（它由部署契约检查覆盖）。失败信息只给 `文件:行:规则:掩码值`，不回显原值——公开仓库的 CI 日志本身也是公开面。豁免只有两种：合成夹具在**同一行**带合成标记（`not-a-real` / `fake` / `dummy` / `synthetic` / `placeholder` / `redacted`），以及少数“说明规则或验证规则所必需”的通用示例坐标，按精确值登记并写明理由；测试会断言每条豁免都写了理由、且仍被命中（过期豁免即失败），因此豁免不会烂成一片放行。
...
`src/review/` 独立于只读 Portal，默认监听 `127.0.0.1:8791`（`PORTICO_REVIEW_PORT`）。`GET /review` 接受 Portico 登录会话或已校验的 Cloudflare Access JWT，然后把浏览器 303 到 `/internal?state=pending_public`——日常审批在内部工作台的内容详情上，Review 不再维护第二套队列界面；`POST /review/approve` 与 `/review/reject` 只接受 human auditor（会话或映射后的名册人类），主体由 session 或已校验 email 查名册得出，不能靠声明头或明文邮箱头。已出示的 Portico 会话永远优先于 JWT；JWT 失败一律匿名，不写 identities/sessions，不签发会话。审批仍由 `CatalogService` 执行，因此自批、非审计者、非法状态和失败无脏写规则不分叉。Review 进程可读 catalog/identities/sessions，只能写 catalog 与 sessions 及其 `.tmp`；启用 JWT 映射时 `up`/`build` 给 Review 进程追加且仅追加该 team 的 JWKS 出站主机（默认关闭则无）。Portal 对目录、名册、审计轨迹与结论保持无写权限；它唯一能写的文件是 `sessions.json`（及其 `.tmp`），因为 `POST /login` 与 GitHub 回调要签发浏览器会话——一行会话记录，不是治理记录。这份授权与 GitHub OAuth 是否启用无关：`POST /login` 一直在，把它绑定在 OAuth 开关上，等于在没有 OAuth 的部署里让唯一的浏览器登录入口写不下去（`403 登录失败`，与「凭证错」无法区分）。`GET /review/login` 与 `POST /review/login` 提供人工会话入口，成功后设置 `Secure; HttpOnly; SameSite=Lax` 的 `portico_session` cookie，不回显 token。Mira 等已认证 maintainer 可 `POST /review/api/submit` 提交 `RegisterInput`，显式 `action: "public"` 才提交公开候选，但不能审批。未出示凭证一律 401；出示了但无效、伪造或落在名册之外的凭证一律 403 `FORBIDDEN`——「证明自己」与「这份证明无效」是两种拒绝，Review 与 Portal/MCP 用同一套语义，因此四入口的拒绝码可以互相对照而不是各自发明。`reviewUrl()` 与 `portalUrl()` / `mcpUrl()` 对称地给出入口地址，E2E 与运维脚本不必硬编码端口；四个入口的监听端口都接受 `0`，使并行测试各自拿到空闲端口、互不抢占。人工既可用一次性凭证在 `/review/login` 登录，也可用 allowlisted GitHub 账号在 `/review/oauth/start` 登录（见 `docs/access/cf-access.md`“Review GitHub OAuth 登录”）；两种方式签发的都是普通 Portico 会话，审批权限仍以名册为准。CLI、Gateway、MCP 仍只认 Portico session，不读 Access 头。
...
公开可达性（匿名能否 `list` / `get` / `describe` / 被 Gateway 路由）不看记录自己的 `visibility` / `governanceState`，而看审批轨迹里该 surface 的最新一条决定。`catalog.json` 只是一份 JSON 文档：它可以被手改、被旧备份覆盖、被写库 bug 写坏，所以记录自称 `approved_public` 不构成公开授权。当记录声称 `approved_public` 而轨迹没有授予时，所有入口都按 `internal` 读取它（并丢掉 `publicSubmission`，与 `withdraw` 留下的形状一致），响应不泄漏入口。反向也要成立：轨迹授予但记录自身已被治理写回到 `internal` 时同样不可见——两半都要满足，这条规则只会收缩、不能扩张可达性。轨迹是追加式的，同一 surface 取最新一条决定；同一毫秒内的决定（撤回后立刻重提、重审是常态）按轨迹追加顺序判定，`reviewedAt` 只用于阻止"随后追加的一条更早时间的决定"覆盖较新的决定。因此直接写库不能公开任何东西，而撤回也不会因为字节被改回 `approved_public` 而失效。四个入口（CLI / Portal / MCP / Gateway）对同一身份呈现同一状态。伪造出来的记录仍可在带内修复：`catalog update` 能把它改好，再走 `publish` + 独立 `approve` 公开，id 不被毒化。

- 公开边界与受众只读报告

`catalog audience --id`、Portal `GET /api/audience/:id` 与 MCP `portico_audience` 共用 `audienceReport`（`src/catalog/audience.ts`），对同一条记录给出同一份报告：`claimed`（记录自身声明的 `visibility` / `governanceState`）、`served`（读路径实际返回的两字段）、`decision`（轨迹对该 surface 的最新一条决定，没有则 `null`）、`reachable`（匿名是否可达），以及按 id 排序的名册受众——每个身份 `{id,kind,role,reachable}`，角色来自名册而不是调用方声明，报告不携带邮箱。`claimed` 与轨迹不一致时报告 `mismatch`：字节自称公开而轨迹没有授予是 `claimed_public_without_approval`，轨迹仍授予而字节已被写回 `internal` 是 `approved_without_public_record`；两种情况下 `reachable` 都是 `false`。这份报告是只读的判断依据，不是第四个入口：它不能改可见性、不能批准，也不替审计者下结论。门禁与 `catalog get` 同一套（读路径共用 `publicGrant` / `underPublicGrant` / `canSee`）：只有维护者与人类审计者可读，只读者与匿名得到 `FORBIDDEN`（Portal 403），草稿对审计者仍是 `NOT_FOUND`，且角色先于取值判定——被拒身份对存在与不存在的 id 得到同一拒绝码，不泄漏 id 是否存在。每个主体的 `reachable` 与其自身 `catalog get` 结果一致，避免名册视图与可见性规则各说一套。

- 公开边界整库巡检

`catalog boundary`、Portal `GET /api/boundary` 与 MCP `portico_boundary` 共用 `boundarySweep()`（名册门禁壳 `src/catalog/boundary.ts`，实现在 `CatalogService`）。单条报告回答「这一条暴露了吗」，整库巡检回答审计者真正要签字的问题：现在到底暴露了什么、每条暴露依据的是哪一次审批、以及哪些可见记录的字节与轨迹不一致。载荷是 `counts`（`visible` / `public_face` / `claimed_public` / `approved` / `mismatched`）、按 id 排序的 `publicFace`（每条暴露入口的 id / name / version / channels / entry，加上它依据的那次决定的 `approvedBy` / `approvedAt`）与同形的 `mismatches`。一条记录只有同时被轨迹授予且字节暴露才进 `publicFace`——暴露由轨迹授予，所以条目必然带着它所依据的那次决定，巡检不会展示一条说不出来源的公开入口。`counts.visible` 与调用者自己的 `catalog list` 条数逐一相等，草稿仍不进审计者的可见集合、也不进任何角色的 `publicFace`：巡检不是新的读权限，只是把已有的可见性与轨迹并排摆出来。门禁与 `catalog audience` 同一套（维护者与人类审计者，只读者与匿名 `FORBIDDEN`），并且只读——不写目录、不追加决定、不是批准入口，也不能用它消除不一致：它只负责把不一致指出来，修复仍必须走审批路径。
...
`src/mcp/` 是 MCP（JSON-RPC 2.0 over HTTP）**服务端**，暴露二十四个只读工具：`portico_list`、`portico_describe`、`portico_entry`、`portico_mcp`、`portico_web`、`portico_cli`、`portico_dashboard`、`portico_audience`、`portico_boundary`、`portico_audit`、`portico_audit_verify`、`portico_seal_anchors`、`portico_approvals`、`portico_identities`、`portico_grants`、`portico_revokes`、`portico_whoami`、`portico_sessions`、`portico_credentials`、`portico_credential_revokes`、`portico_gateway_audit`、`portico_conclusions`、`portico_conclusion_standings`、`portico_page`。每个工具都是 `CatalogService` / `AuditService` / `AccessService` / `PageService` / `GatewayService` 已有调用的薄投影，因此可见性、审批与角色规则不会分叉——同一身份在 MCP、CLI、Portal 上看到的是同一批记录、同一顺序。工具结果是 CLI 同一个 `{ok,data}` / `{ok,error:{code,message}}` 信封，所以"三入口一致"可以靠比对载荷验证，而不是靠读三份实现。鉴权**只认会话**（`Authorization: Bearer` / `X-Portico-Session`）：网络调用者不得靠自称头证明身份。协议层错误（未知方法 `-32601`、未知工具 `-32602`、批量请求 `-32600`、解析失败 `-32700`）与工具层失败（in-band `isError`）分开；非 POST 返回 405。`portico_audit` 仍只对人类审计者开放；`portico_approvals` 对已登录身份开放、匿名得到空列表；`portico_identities` 对维护者与人类审计者开放，只读与匿名 FORBIDDEN；`portico_grants` 仅人类审计者，维护者/只读/匿名 FORBIDDEN；`portico_revokes` 仅人类审计者，维护者/只读/匿名 FORBIDDEN；`portico_whoami` 对已登录身份开放、匿名 FORBIDDEN；`portico_sessions` 仅人类审计者，维护者/只读/匿名 FORBIDDEN；`portico_credentials` 仅人类审计者，维护者/只读/匿名 FORBIDDEN；`portico_credential_revokes` 仅人类审计者，维护者/只读/匿名 FORBIDDEN；`portico_gateway_audit` 仅人类审计者，维护者/只读/匿名 FORBIDDEN；`portico_audience` 与 `portico_boundary` 对维护者与人类审计者开放，只读与匿名 FORBIDDEN；`portico_web` 对匿名与已登录身份开放，内部 Web 对匿名不可见；`portico_cli` 对匿名与已登录身份开放，内部 CLI 对匿名不可见；`portico_page` 对匿名与已登录身份开放，内部卡片对匿名不可见。MCP 进程只读（无 `--allow-write`），不执行、不代理、不编排任何外部工具。
...
`src/audit/` 给人类审计者一条只读时间线。`audit list`、Portal `GET /api/audit` 与 MCP `portico_audit` 对同一身份合并同一份时间线：追加式目录变更（register / draft / publish / update）、身份授权、身份撤回、登录凭证作废、公开审批，以及 Gateway 访问审计。三处都读同一个可选 `PORTICO_GATEWAY_AUDIT_PATH`；`up` 会把它同时交给 Portal、MCP 与 Gateway，所以默认情况下三者呈现相同内容，而不是"CLI 有、其他入口没有"。`audit list` 接受 `--q` / `--kind` / `--action` / `--subject`，与 Portal `GET /api/audit`、MCP `portico_audit` 共用 `src/audit/query.ts`：过滤发生在审计者鉴权之后，只匹配 id / 主体 / 摘要 / 动作，不搜索入口 URL 或包坐标；非法过滤器 `INVALID_INPUT`，失败不写目录。维护者、只读者与匿名即使带过滤器也得到 `FORBIDDEN`；没有改写或删除入口。同一个入口还能把「读取」本身收窄到一个过去时刻：`audit list --as-of <instant>`（Portal `GET /api/audit?asOf=`、MCP `portico_audit` 的 `asOf`）只读这条追加式轨迹里截至该时刻已经存在的记录。切片不是第二份存储、不是快照、更不是改写入口——它是读得少一点的同一条轨迹，所以历史状态无法被伪造，也不需要为「当时怎么样」额外保留一份可能撒谎的副本。`asOf` 只接受带时区的真实瞬时：裸日期（`2026-09-21`）、无时区墙钟（`2026-09-21T12:00:00`）、自然语言（`now` / `yesterday`）、不存在的日期（`2026-02-30`）、`T24:00` 与越界偏移都被 `INVALID_INPUT` 拒绝，因为一个猜出来的窗口看起来和一个正确的窗口一模一样。无法解析时间戳的记录不进任何窗口（不被猜进时间线），不切片时仍照常出现。Portal `/internal/audit` 是无脚本 GET 表单，只给人类审计者（表单里的 `asOf` 字段把时间线与顶部判定面板一起切到那个时刻，并给出「回到当前窗口」链接；两块被切的面板与时间线都标明自己回答的是 `as-of` 窗口，切片时带上 `data-asof`），并在同一页面上渲染封条完整性面板：面板标明自己的窗口是 `current` 并写明读取窗口不作用于它——封条校验没有历史模式，切片时把截止时刻渲染成 `<time datetime="…">`，于是切片页上的「封条 ok」不会被读成「那一刻是好的」；面板与 `GET /api/audit-verify` 用同一份报告（同一判决、同一批未封记录、同一批锚点状态），逐支柱列出结论与 tip，指名断链的 (seq / kind / id) 与原因，并区分"链上缺口"（点名链上的洞）与"检查点判定的截断"（归因检查点）这两种不同指认；未配置的支柱标成未校验，不把"没查"画成"通过"。这条读取路径不因为磁盘上一条坏记录而消失：缺嵌套引用（`actor` / `grantedBy` / `revokedBy` / `reviewedBy` / `endpoint`）的记录计为未证实主体（`anonymous`），而不是让整页 `INTERNAL`——数据文件被写坏不该决定审计面是否可见。Portal 写方法仍 405，失败登记不写目录变更。审计结论是另一份独立存储里的独立能力（见下条「安全审计结论」），维护者身份不能覆盖，也不能把自己的维护动作当成审计意见。

- 安全审计结论（维护权 ≠ 审计权）

`src/audit/conclusions.ts` 让人类审计者把安全判断写成追加式的结论记录，存在与维护轨迹分开的独立文件里。`audit conclude --id <subject> --scope <scope> --verdict <verdict> [--note <text>]` 记录一条结论：作用域固定为 `public_boundary` / `entry_target` / `permission_change` / `secret_leakage` / `gateway_scope` / `runtime_l0`（每条结论恰好回答一个问题），结论固定为 `cleared` / `flagged`。只有人类审计者能写：维护者、只读者、匿名与 Agent 一律 `FORBIDDEN`——Agent 连 auditor 角色都拿不到（`identity grant --kind agent --role auditor` 被拒），所以伪造 actor 是唯一入口，而它在这里同样被拒。审计者若是该主体的维护者之一，得到 `SELF_AUDIT`：**维护权不等于审计权**，与公开边界的 `SELF_APPROVAL` 同源，只是主体换成维护者名单。结论是追加式的：没有 update、没有 delete，复评只追加新记录并保留旧结论可读（"审计者过去怎么判"本身是证据），重复 id 得到 `ALREADY_EXISTS` 而不是覆盖。`audit conclusions`、Portal `GET /api/conclusions` 与 MCP `portico_conclusions` 读同一批记录（三者共用同一结论文件；Portal 与 MCP 仍然只读，记录只能经 CLI 写入）。过滤 `--subject` / `--scope` / `--verdict` 发生在审计者鉴权之后，只匹配主体 id 与两个枚举字段，非法过滤器 `INVALID_INPUT`。未知主体 `NOT_FOUND`（对不存在的东西下判定不是证据）。`flagged` 必须带 note——没有描述的发现不可执行；note 超过 500 字符、含控制字符、或含明文密钥（沿用目录同一套扫描器，含 AWS Access Key ID `AKIA`/`ASIA` 形值、Google API Key `AIza` 形值、PEM 私钥头与 npm 令牌 `npm_` 形值）都被拒。被拒的结论不写结论文件、不改目录、也不产生目录变更事件；只读查询不创建该文件。`audit standings`（Portal `GET /api/conclusions/standings`、MCP `portico_conclusion_standings`）把这条轨迹折成「此刻的判定」：每个主体与作用域取最新一条结论作为当前判定，并带回它来自哪条记录、上一条判定是什么（区分「曾被标记、后已清除」与「从未被标记」）以及该主体该作用域下 `cleared` / `flagged` 的计数。过滤作用于**当前判定**而不是轨迹：已清除的旧标记不会命中 `verdict: flagged`，而轨迹本身仍把它当证据保留；空列表表示还没有审计者作出判定，不是「全部通过」。同一个 `asOf` 也能给判定面板：给一条结论的写入时刻（含该时刻）就得到那一刻的判定——当时判 `flagged` 就是 `flagged`，不受此后清除影响——省略则给此刻的判定，晚于全部记录的截止时刻与不切片同载荷（三入口与 `/internal/audit` 共用 `src/audit/instant.ts` 的同一份切片语义）。

其中两个问题问的不是某一条登记表面，而是仓库整体：Deno L0 运行时边界（`runtime_l0`）与公开面脱敏。它们没有目录记录可以挂判定，也不该为此造一条假记录，因此是自带契约的**边界主体**：`boundary:runtime-l0`（回答 `runtime_l0`，门禁 `deno task check:runtime-boundary`）与 `boundary:public-redaction`（回答 `secret_leakage`，门禁 `deno task check:redaction`）。每个边界主体只回答一个问题，并指向回答它的门禁任务——结论上因此多一个 `gate` 字段，判定与可重跑的证据绑在一起，而不是一句无从复核的表态。

边界 id 与表面 id 属于两个不会互相冒充的命名空间：表面 id 是小写 kebab-case，边界带 `boundary:` 前缀，所以带前缀的 id 注册成表面被拒；反过来 `runtime_l0` 不能用在对表面的判定上，表面 id 也不能当边界主体（`INVALID_INPUT`），长得像边界但并不存在的 id 得到 `NOT_FOUND`。`secret_leakage` 是两个面共用的问题——表面记录问「这条登记泄漏了吗」，`boundary:public-redaction` 问「整个公开面泄漏了吗」——这正是脱敏契约既要有逐条判定、也要有一次整体签核的原因。边界结论只写结论文件：不写目录、不进维护轨迹、不改变任何对象的可见性，它是意见而不是治理事件。
...
Web 入口用于浏览目录、查看治理状态、进入被授权的 Agent 表面。仪表盘默认回答治理问题（有什么、是否公开、待审多少、最近谁改了），不为 Agent 运行指标负责。页眉上的审核入口属于部署事实而非 UI 常量：三个入口的内网测试部署不运行独立审核服务，因此由 `PORTICO_REVIEW_ORIGIN` 声明本部署对外提供哪个审核入口——声明 `off` 时审核入口整体不出现，声明绝对 origin 时链到该 origin，未声明时按同源 `/review`（macOS 反代部署的形状）；非法值一律按「不提供」处理，Portal 不探测也不猜测对端是否存在，免得把 404 挂成治理路径。同一份声明同时约束只读部署门禁。
...
Registry 内部登记、Publisher 草稿/内部发布/公开候选、Approval 通过/拒绝、公开发布撤回、公开边界与受众只读报告、Access Control 身份名册、身份名册只读查询、当前会话身份只读查询、登录会话只读查询、登录凭证只读查询、登录凭证作废轨迹只读查询、身份撤回轨迹只读查询、名册可选邮箱、身份授权撤回、登录会话、登录凭证作废、Portal 可选 Cloudflare Access JWT 映射、Portal 发现/仪表盘、Portal 双平面 UI 与颜色主题、Review 独立人类审核入口（提交者不能自审、未审批不可公开、拒绝码与其它入口一致）、目录过滤查询、审计时间线过滤查询、审计时间切片读取（截至某时刻重建轨迹与判定，错误的窗口宁拒不猜）、MCP 渠道登记/连接信息、Web 渠道登记/已授权入口、CLI 渠道登记/已授权包坐标、MCP Gateway 鉴权路由、人类安全审计视图、安全审计结论（人类审计者对公开边界/入口指向/权限变更/密钥泄漏/Gateway 作用域写下追加式判定，只有人类审计者能写、审计者不能审自己维护的主体）、UI Components 门户页维护，以及 CLI 对应命令已有测试证据。邮箱仍不是第二身份证明。三个非匿名入口里，CLI / Gateway / MCP 仍只认会话：CLI `--session`，Gateway / MCP 的 `Authorization: Bearer` 或 `X-Portico-Session`。Portal 在默认关闭时同样只认会话；显式启用 Cloudflare Access 后，校验通过的 JWT 只映射名册，不签发会话。`--actor-*` 与 `X-Portico-Actor-*` 不构成证明，Portal / Gateway / MCP 已不接受，CLI 单独给出即拒绝。GitHub OAuth / 邮箱 OTP 界面与 Tunnel 仍是边缘基础设施，不在本仓库。
...
| 删除回收站 | 高 | ✅ 维护者 `catalog remove` 后记录离目录进回收站，可 `catalog restore` 回到删除前状态；人类审计者 `catalog purge` 后永久消失 | ✅ 待审/已公开记录删除被拒（`INVALID_STATE`，须先驳回/撤回）；维护者永久删除与只读/匿名读回收站 `FORBIDDEN`；未知 id `NOT_FOUND`；已在回收站的 id 不可重新登记 | ✅ 人类审计者 vs 维护者 vs 只读/匿名 | ✅ 失败删除/恢复/永久删除不改目录文件；恢复只回删除前状态不重发公开；`remove`/`restore`/`purge` 全进变更轨迹 | `tests/catalog_remove_test.ts`；`tests/e2e/cli_trash_e2e_test.ts` |
| 公开可达性以审批轨迹为准 | 高 | ✅ 维护者 `publish --visibility public` + 独立人类审计者 `approve` 后，匿名在 CLI `catalog list`、Portal `GET /api/catalog` 与 `/public/s/:id`、MCP `portico_list`、Gateway `POST /gateway/mcp/:id/authorize` 同时可达同一条 `approved_public` | ✅ 直接把 `visibility=public` + `governanceState=approved_public` 写进 `catalog.json`（轨迹里没有任何审批）后，匿名在四个入口全部不可达（CLI 空列表、Portal `/api/catalog` 空列表且 `/public/s/:id` 404、MCP 空列表、Gateway authorize 404 `NOT_FOUND`），MCP 入口不因记录自称公开而出现；撤回后再把字节改回 `approved_public` 仍不可达（轨迹最新一条是 `withdrawn`）；自批 `SELF_APPROVAL` 仍被拒 | ✅ 匿名（四个入口均不可达）vs 已登录只读（同一批记录、同一顺序，且两条入口都报 `internal`）| ✅ 伪造记录可带内修复：`catalog update` → `publish` → 独立 `approve` 后公开，id 不被毒化；被拒的匿名路由仍写入一条 `denied` 网关审计（拒绝留痕，不是静默忽略）| `tests/catalog_public_grant_test.ts`；`tests/e2e/public_grant_e2e_test.ts` |
| 公开边界与受众只读报告 | 高 | ✅ 同一维护者与同一人类审计者经 CLI `catalog audience --id`、Portal `GET /api/audience/:id` 与 MCP `portico_audience` 得到同一份报告：`claimed` / `served` / `decision` / `reachable` 与按 id 排序的名册受众（`{id,kind,role,reachable}`，角色来自名册）；已审批公开条目三入口一致（`served.visibility=public`、`decision=approved`、`reachable=true`、全部已登录身份可达）；`withdraw` 之后三入口一致读到 `served.visibility=internal`、`decision=withdrawn`、`reachable=false`、已登录身份仍可达 | ✅ 只读者与匿名在三入口均 `FORBIDDEN`（Portal 403），且对不存在与存在的 id 得到同一拒绝码（不泄漏 id 是否存在）；草稿对审计者三入口 `NOT_FOUND`，只有维护者能读到 `served.governanceState=draft`；把 `approved_public` 直写进 `catalog.json` 而轨迹无审批时报告 `mismatch=claimed_public_without_approval`、`served.visibility=internal`、`reachable=false`；轨迹仍有 `approved` 而字节被写回 `internal` 时报告 `mismatch=approved_without_public_record` | ✅ 维护者/人类审计者（可读）vs 只读者/匿名（`FORBIDDEN`）；草稿只对维护者可见 | ✅ 读报告不改 catalog 文件字节、不追加审批记录（轨迹与目录字节前后一致）；每个名册主体的 `reachable` 与其自身 `catalog get` 结果逐一一致；报告不携带名册邮箱 | `tests/catalog_audience_test.ts`；`tests/e2e/catalog_audience_e2e_test.ts` |
| 公开边界整库巡检 | 高 | ✅ 同一维护者与同一人类审计者经 CLI `catalog boundary`、Portal `GET /api/boundary` 与 MCP `portico_boundary` 得到同一份巡检：`counts`（`visible` / `public_face` / `claimed_public` / `approved` / `mismatched`）、按 id 排序的 `publicFace`（每条暴露入口的 entry / channels / version，以及它依据的那次审批的 `approvedBy` / `approvedAt`）与 `mismatches`；已审批公开条目进清单，未暴露条目不进；`counts.visible` 与同一角色自己的 `catalog list` 条数相等——巡检不放大任何角色的可见范围 | ✅ 字节自称 `public` + `approved_public` 而轨迹无审批时报 `claimed_public_without_approval`（`served` 落回 `internal`、`reachable=false`、`decision=null`，匿名读该条仍 `NOT_FOUND`）；轨迹仍 `approved` 而字节被写回 `internal` 时报 `approved_without_public_record`；`withdraw` 之后清单与 `mismatches` 同时归零 | ✅ 维护者/人类审计者（可读）vs 只读者/匿名（三入口 `FORBIDDEN`，Portal 403）；草稿不进审计者的 `visible` 计数，也不进任何角色的 `publicFace` | ✅ 巡检前后 catalog 文件字节与审批轨迹均不变（不追加决定、不改可见性）；拒绝先于读目录，被拒身份在任何目录状态下得到同一 `FORBIDDEN` | `tests/catalog_boundary_sweep_test.ts`；`tests/e2e/catalog_boundary_e2e_test.ts` |
| Review 独立人类审核入口 | 高 | ✅ 维护者 `POST /review/api/submit`（`action: "public"`）提交后该条只是 `pending_public`；人类审计者浏览器式 `POST /review/login` 取得 `Path=/; Secure; HttpOnly; SameSite=Lax` 的普通 Portico 会话后，`GET /review` 303 到 `/internal?state=pending_public`（日常审批在内容详情上，Review 不再维护第二套队列界面），详情页上的 `POST /review/approve` 或 CLI `catalog approve` 都能完成审批，匿名随即在 Portal `/public/s/:id`、Portal `GET /api/catalog`、CLI `catalog list` 与 MCP `portico_list` 四处看到同一条；审批轨迹 `reviewedBy=human:security-auditor`、`submittedBy=agent:docs-bot` | ✅ 匿名 `GET /review` 401；Agent 维护者（提交者）、人类维护者、只读者与未知会话 approve 403 `FORBIDDEN`；伪造或过期 Access 断言带手写邮箱头 401，名册外合法断言 401；缺 id 400、未知 id 404；未审批时 Portal 公开页仍 404；非 human auditor 走浏览器登录入口被拒 | ✅ 匿名 vs 只读 vs 维护者（Agent 与人类）vs 人类审计者；提交者不能自审 | ✅ 被拒调用后 catalog 与 identities 字节不变、公开页仍 404、候选留在队列并可被真审计者完成审批 | `tests/review_handler_test.ts`；`tests/portal_review_entry_test.ts`；`tests/e2e/review_http_e2e_test.ts`；`tests/e2e/system_up_e2e_test.ts` |
| Portal 发现与仪表盘 | 中 | ✅ CLI register 后 reader 在 Portal list/HTML 与 `/s/:id` 看到同一条；approve 后匿名在 hero/list/详情看到同一条 `approved_public`；同一身份经 CLI `catalog dashboard`、Portal `GET /api/dashboard` 与 MCP `portico_dashboard` 得到同一计数与同一顺序；无 `channel` 的详情页顶栏不默认 Web，「内容」高亮；阅读页只保留一组渠道入口且回到带筛选的列表；渠道展示名来自 `CHANNEL_LABEL`；面包屑渠道层链到带筛选的列表；`q` 透传到左栏渠道入口、面包屑首页与面包屑渠道层；可见记录上不匹配的 `channel`/`q` 会 302 到自身渠道（`q` 仍命中则保留），规范化后再渲染时列表包含选中记录；杂志壳链到 `/public`，已登录身份另链到 `/internal`，并把未过滤的 `pending_public` 计数链到 `/internal/pending`（`q`/`channel` 过滤不缩小该计数）；公开发布面链回杂志 `/`，公开详情链到同一条 `/s/:id`；阅读页「返回列表」保留 `channel`/`q` | ✅ 内部与 pending_public 对匿名不可见；渠道过滤不泄漏匿名不可见记录；内部详情对匿名 HTML 404 且不泄漏入口；匿名带错误 `channel` 访问内部详情仍 303 到登录、Location 不泄漏；匿名杂志壳与公开面都不出现 `/internal` 或 `/internal/pending`；匿名 `/internal` 303 到 `/login?next=…` 且响应里没有记录名、入口或计数；POST 405；冒充 auditor FORBIDDEN；HTML 转义名称；缺 `--catalog` 的 dashboard 为 `USAGE` | ✅ reader vs 匿名 | ✅ 失败写不改 catalog 文件；读 dashboard 不改目录字节；阅读页筛选规范化不改目录字节；跨面导航是只读链接，不改目录字节；Portal 与 MCP 都不写目录/名册/审计/结论（Portal 唯一的写授权是 `sessions.json`，见「运行时边界」） | `tests/portal_handler_test.ts`；`tests/portal_magazine_test.ts`；`tests/portal_ui_test.ts`；`tests/catalog_dashboard_test.ts`；`tests/e2e/portal_discovery_e2e_test.ts`；`tests/e2e/portal_magazine_e2e_test.ts`；`tests/e2e/catalog_dashboard_e2e_test.ts` |
...
| Agent 维护与人类安全审计 | 高 | ✅ 维护者登记并提交公开后，人类审计者 `audit list`、Portal `GET /api/audit` 与 MCP `portico_audit` 看到同一条目录变更、授权、撤回与审批时间线；配置 `PORTICO_GATEWAY_AUDIT_PATH` 后三处都并入 Gateway 访问事件 | ✅ 维护者/只读/匿名 FORBIDDEN；Portal PATCH/POST `/api/audit` 405；失败公开登记不出现 catalog 事件；磁盘上缺嵌套引用（`actor` / `grantedBy` / `revokedBy` / `reviewedBy` / `endpoint`）的记录仍出现在时间线上并计为未证实主体（`anonymous` / `unknown`），不让整页 `INTERNAL` | ✅ 维护 Agent vs 人类审计者；维护者不能读、不能改写 | ✅ 失败登记不写 catalog 文件与变更日志；读审计不改授权/审批记录 | `tests/catalog_change_test.ts`；`tests/audit_service_test.ts`；`tests/portal_handler_test.ts`；`tests/e2e/cli_audit_e2e_test.ts`；`tests/e2e/portal_audit_e2e_test.ts`；`tests/e2e/portal_integrity_e2e_test.ts` |
| 安全审计结论（维护权 ≠ 审计权） | 高 | ✅ 人类审计者 `audit conclude --scope public_boundary --verdict flagged --note ...` 后，`audit conclusions`、Portal `GET /api/conclusions` 与 MCP `portico_conclusions` 看到同一条；审计者读得到自己名下另一作用域的结论（独立性按主体判定，不按结论）；仓库级边界契约同样可签核：对 `boundary:runtime-l0` 下 `--scope runtime_l0` 后，CLI/Portal/MCP 读到同一条并带 `gate: check:runtime-boundary`（脱敏契约 `boundary:public-redaction` 同理，`gate: check:redaction`）；复评后 `audit standings`、`GET /api/conclusions/standings` 与 `portico_conclusion_standings` 三入口同载荷地给出此刻的判定：当前 `verdict`、它来自哪条结论（`conclusionId` / `auditorId`）、上一条判定 `previousVerdict: flagged`（「曾被标记、后已清除」）与该主体该作用域下 `cleared` / `flagged` 计数，`/internal/audit` 顶部渲染同一批结果且面板内无表单、无脚本 | ✅ 维护者/只读者/匿名读写均 `FORBIDDEN`；Agent 声称 auditor 角色被拒（`identity grant` 与结论服务两处）；审计者同时是该主体维护者时 `SELF_AUDIT` 且不落盘；未知主体 `NOT_FOUND`；非法 scope/verdict/未知字段/密钥字段被拒；`flagged` 无 note 被拒；note 含明文密钥（AWS Access Key ID、Google API Key、PEM 私钥头、npm 令牌形值）或控制字符被拒；重复 id `ALREADY_EXISTS`；损坏文件响亮失败；边界主体只接受自己契约的作用域（表面作用域或另一契约的作用域均 `INVALID_INPUT`）、`runtime_l0` 不能用在对表面的判定上、带 `boundary:` 前缀的 id 不能注册为表面、未知边界 id `NOT_FOUND`、边界主体同样拒绝维护者与非审计者；standings 的角色检查先于过滤器解析（非审计者得到 `FORBIDDEN` 而不是 `INVALID_INPUT`），过滤器只看当前判定：已清除的旧标记不命中 `verdict: flagged`，而轨迹查询仍返回它 | ✅ 人类审计者 vs 维护者 / 只读者 / 匿名；审计者兼维护者被拒；三入口的 standings 与 `/internal/audit` 对维护者 / 只读者 / 匿名均 `FORBIDDEN`（页面对非审计者 HTML 404） | ✅ 被拒结论不写结论文件、不改目录、不产生目录变更事件；读不创建文件；复评追加而非覆盖（旧结论仍可读）；边界结论只写结论文件（不写目录、不进维护轨迹、不改可见性）；standings 是纯派生读：读不写结论文件、`POST /api/conclusions/standings` 返回 `405`，空轨迹渲染为「还没有留下判定」而不是通过 | `tests/audit_conclusions_test.ts`；`tests/e2e/audit_conclusions_e2e_test.ts`；`tests/e2e/audit_standings_e2e_test.ts` |
...
| 审计时间切片读取（截至某时刻重建） | 高 | ✅ 同一人类审计者给出同一个 `asOf`（带时区的真实瞬时，如 `2026-09-21T12:00:00Z`）时，CLI `audit list --as-of` / `audit conclusions --as-of` / `audit standings --as-of`、Portal `GET /api/audit?asOf=`、`GET /api/conclusions?asOf=`、`GET /api/conclusions/standings?asOf=` 与 MCP `portico_audit` / `portico_conclusions` / `portico_conclusion_standings` 的 `asOf` 得到同一批结果与同一顺序；窗口以记录自身时间戳界定（`asOf` 为该条结论的写入时刻时含该条）：在首条结论时刻切片看到的就是当时的判定（`flagged`、`count=1`、无 `previousVerdict`），在末条结论时刻或省略 `asOf` 时看到今天的判定（`cleared`、`previousVerdict: flagged`、`count=2`），晚于全部记录的截止时刻与不切片同载荷；窗口内没有任何结论时返回空列表，晚于某条记录的截止时刻不再包含它；`/internal/audit?asOf=` 顶部面板带 `data-asof`、标题写「当时判定」并给出「读取窗口：截至 …」一行与「回到当前窗口」链接，表单同样带 `asOf` 字段与「必须带时区」提示，窗口早于全部事件时页面写「该时刻没有…」而不是「还没有留下判定」 | ✅ 只有日期（`2026-09-21`）、无时区的墙钟（`2026-09-21T00:00:00`）、自然语言（`yesterday` / `now`）、不存在的日期（`2026-02-30`、`2025-02-29`）、`T24:00`、越界偏移、超长与控制字符、非字符串值一律 `INVALID_INPUT`，不落回「现在」（错误的窗口与正确的窗口无法区分，所以宁拒不错读）；角色门先于过滤器解析：维护者/只读者/匿名带上非法 `asOf` 仍得到 `FORBIDDEN`，不泄漏过滤器文法；非审计者 `/internal/audit?asOf=` 仍是 HTML 404；`POST /api/conclusions/standings?asOf=` 405；无法解析时间戳的记录不进任何窗口（不被猜进时间线），不切片时仍照常出现 | ✅ 人类审计者 vs 维护者/只读者/匿名；三入口与 `/internal/audit` 对维护者/只读者/匿名均拒绝，页面按不存在处理 | ✅ 切片是纯派生读：读不写结论文件、不改目录字节、不创建文件（含全部被拒请求）；失败不掉期、不改写任何记录，历史状态只由读得少一点的同一条追加式轨迹得到 | `tests/audit_instant_test.ts`；`tests/audit_query_test.ts`；`tests/audit_conclusions_test.ts`；`tests/e2e/audit_timeslice_e2e_test.ts` |
| 审计读取窗口声明（封条不切片，宁拒不猜） | 中 | ✅ 同一人类审计者经 CLI `audit verify`、Portal `GET /api/audit-verify` 与 MCP `portico_audit_verify` 拿到同一份报告，而报告自己先声明窗口：载荷首个字段是 `window: "current"`——封条重算的是此刻磁盘上的文件，没有「截至某时刻的封条」这种读法。省略截止时刻、显式传空值、`--as-of ''` 三种「没有窗口」的写法得到字节相同的报告，所以「没有窗口」只有一种含义；`/internal/audit` 上封条面板带 `data-window="current"`，判定面板与时间线带 `data-window="as-of"`（切片时时间线另带 `data-asof`）；切片页面上封条面板渲染「读取窗口不作用于这一面板：封条校验没有历史模式」并把截止时刻渲染成 `<time datetime="…">`，因此切片页上的「封条 ok」不会被读成「那一刻是好的」；当前窗口、切片到全部记录之后、窗口内无记录三种页面上，封条面板的读数（`data-integrity` / `data-sealed` / `data-unsealed` / `data-anchored`）不变 | ✅ 封条拒绝回答不了的问题，不静默忽略：CLI `audit verify --as-of <instant>`、Portal `GET /api/audit-verify?asOf=`、MCP `portico_audit_verify` 的 `asOf` 与直接调用服务都得到 `INVALID_INPUT`（带时区的真实瞬时、裸日期、`now` 与非字符串一律如此），因为一个被丢掉的截止时刻和一个没有作用的截止时刻从外面看完全一样——把 `asOf` 悄悄扔掉会让人以为读到的是历史（错误与正确无法区分，所以宁拒不猜）；不带窗口的三种写法仍照常返回同一份报告，读取路径不因拒绝而消失；`POST /api/audit-verify` 仍是 `405` | ✅ 人类审计者 vs 维护者/只读者/匿名/无法读取的 actor；角色门先于窗口文法：非审计者带上非法 `asOf` 仍得到 `FORBIDDEN`，不泄漏窗口文法；不在名册里的主体打开 `/internal/audit` 仍是 HTML `404`，页面按不存在处理 | ✅ 全部读取（含被拒的）都是纯派生读：目录、身份名册、Gateway 审计与结论文件字节不变，不创建文件、不改动任何记录，失败不留脏写 | `tests/audit_window_test.ts`；`tests/portal_ui_test.ts`；`tests/e2e/audit_window_scope_e2e_test.ts` |
| 审计封条校验（篡改可指名） | 高 | ✅ 同一人类审计者经 CLI `audit verify`、Portal `GET /api/audit-verify` 与 MCP `portico_audit_verify` 得到同一份四支柱报告（catalog / identities / gateway / conclusions）：每条已封记录按顺序复算摘要链，`ok:true`、`unsealed:0`，三入口报出同一 tip；同一审计者打开 Portal `/internal/audit` 时在 HTML 上看到同一份报告（整体判决、逐支柱 tip 与状态、未封计数），面板不写任何文件 | ✅ 改动、删除或剪断链上任一记录都会点名该条（seq / kind / id）与原因（`digest` / `missing` / `chain`），不笼统报失败；未封记录如实计入 `unsealed` 而不算已通过；校验本身不得把被改记录改回原状；被篡改的链之外，磁盘上被写坏的记录不把审计页变成错误页 | ✅ 人类审计者 vs 维护者/只读/匿名（后三者 FORBIDDEN） | ✅ 校验只读：跑过 CLI / Portal / MCP 三入口后四个支柱文件字节不变；读完 `/internal/audit` 后五个文件字节同样不变；把被改字节还原后结论重新 `ok:true` | `tests/audit_seal_test.ts`；`tests/portal_ui_test.ts`；`tests/e2e/audit_seal_e2e_test.ts`；`tests/e2e/portal_integrity_e2e_test.ts` |
| 审计封条外部锚定（整体重写与尾部截断可发现） | 高 | ✅ 人类审计者经 CLI `audit anchor` 钉住四支柱的 (pillar, seq, tip)；此后 CLI `audit verify --anchors`、Portal `GET /api/audit-verify` 与 MCP `portico_audit_verify` 报出同一份锚定结论（`anchored` 计数与每支柱 `anchor.state`），Portal `GET /api/seal-anchors` 与 MCP `portico_seal_anchors` 列出同一批检查点；未钉检查点时如实报 `anchored: 0`，不把未锚定的支柱冒充成已校验；同一审计者在 Portal `/internal/audit` 的 HTML 上看到同一份锚点状态，并把检查点判定的截断与链上缺口标成两种不同指认 | ✅ 记录与链条被整体重写（摘要全部重算、封条自洽、`ok:true`）被指名 `rewritten`；从尾部截断被指名 `truncated`；锚定过的 tip 出现在链上别处被指名 `moved`；链条正常增长不算篡改（仍 `intact`） | ✅ 人类审计者 vs 维护者/只读/匿名（后三者对 `audit anchor` / `audit anchors` 均 FORBIDDEN，且被拒的钉点不产生锚点文件）；Portal 与 MCP 只读，`POST /api/seal-anchors` 405 | ✅ 比对与列举全程只读：锚点文件与四支柱文件字节不变；把改动过的记录与链条还原后同一检查点重新 `intact`；第二个检查点追加在第一个之后，不覆盖既有证据 | `tests/audit_anchor_test.ts`；`tests/portal_ui_test.ts`；`tests/e2e/audit_anchor_e2e_test.ts`；`tests/e2e/portal_integrity_e2e_test.ts` |
...
| 部署契约与入口健康门禁 | 中 | ✅ 只读门禁先断言验证的是哪个地址（`PORTICO_DEPLOY_BIND` 声明，未声明时取监听表里唯一的那个地址；同端口有两个就 FAIL 而不猜），且三个入口都不绑在所有接口上；再打三个真实入口：发现页（按发现壳结构判产品页，不只看 200）、`/public` 200、`/internal` 404、目录 `ok:true` 信封、Portal 写方法 405、Gateway 工具调用 POST 405、Gateway 审计匿名 403、MCP `initialize` 返回 `portico`、MCP GET 405、三个入口都报出启动器记录的 `PORTICO_REVISION` 且与检出 HEAD 相符（`running-revision`；门禁只取这一个变量，其余启动环境不回显）、监听进程不比它服务的树更旧、`PORTICO_EXPECT_SHA` 与检出相符时报该条 `ok`；页眉只挂出本部署声明服务的审核入口——声明 `off`（三入口测试部署的默认）时产品页不得出现 `/review` 链接，声明 origin 时链到该 origin | ✅ 入口不在声明的地址上（点名声明的地址与真正应答的地址）、端口无人监听（点名端口）、同一端口两个监听地址（拒绝猜测；声明其中一个也仍 FAIL 并点名另一个）、入口绑在所有接口上、入口停机、错误页冒充产品页（同 `<title>` 同壳）、匿名 `/internal` 返 200、监听进程早于树的写入、`PORTICO_EXPECT_SHA` 与检出不符、**没钉修订且未显式接受**（不设 `PORTICO_DEPLOY_ALLOW_UNPINNED`，或设成 `0`/`no` 一类非真值）——报 `FAIL checkout-revision` 并点名两个变量名、入口报出的启动修订与检出不符（点名两个修订并要求重启）、入口没记 `PORTICO_REVISION`（点名该变量并要求重启）、产品页挂出本部署不提供的审核入口（声明 `off` 却出现 `/review`，或声明了 origin 却不链过去）——每条按名字报 `FAIL` 且非零退出 | ✅ 不适用：门禁只用匿名只读探测，不需要身份也不需要 sudo；Portal 与 MCP 仍不持 `--allow-write`，门禁不碰名册 | ✅ 门禁自身无副作用：失败只报名字与非零退出，不重启、不改目录、不留残文件（临时戳只在 `$TMPDIR`）；`skip` 只在显式接受（`PORTICO_DEPLOY_ALLOW_UNPINNED` 取真值）且该条确实不适用时出现，且不冒充通过 | `tests/e2e/deploy_verify_e2e_test.ts`；`tests/deploy_contract_test.ts` |

| macOS 生产部署门禁与运行修订 | 中 | ✅ 只读门禁在宿主机上回答四件事：检出修订（`PORTICO_EXPECT_SHA` 与检出相符时报该条 `ok`）、四个入口（Portal 8788 / Gateway 8789 / MCP 8790 / Review 8791）**是不是由这个检出启动的**（`running-revision`：读回启动器导出的 `PORTICO_REVISION`，只取这一个变量、不回显其余启动环境）、**各自由哪个进程服务**、每个入口各自兑现契约——Portal `/public` 200、Review `/review/login` 200、公网 origin 上两条对应探测、匿名目录 `ok:true` 信封、匿名 `/internal` 404、Gateway 工具调用 POST 405、MCP `initialize` 返回 `portico`；默认值就是本机部署形状（回环 + 一个公网 origin），四个端口与 origin 都可由环境覆盖，因此门禁本身可以被指向测试自己的监听者并跑出全绿 | ✅ 入口没有进程在服务（点名入口与端口，且是 FAIL 而非 `skip`）、入口报出的启动修订与检出不符（点名两个修订并要求重启）、入口没记 `PORTICO_REVISION`（点名该变量并要求重启）、监听进程比 `PORTICO_DEPLOY_TREE/src` 里的代码旧（点名进程与它先于哪个文件写出）、`PORTICO_EXPECT_SHA` 与检出不符、**没钉修订且未显式接受**（不设 `PORTICO_DEPLOY_ALLOW_UNPINNED`，或设成 `0`/`no` 一类非真值）——报 `FAIL checkout-revision` 并点名两个变量名、Gateway 真的执行了工具、MCP 返回 200 但不是 portico 的 initialize 结果、检出里没有 `src` 可比、`mktemp` 不可用——每条按名字报 `FAIL` 且非零退出 | ✅ 不适用：门禁只用匿名只读探测，不需要 sudo，不读名册也不改目录状态 | ✅ 门禁自身无副作用：失败只报名字与非零退出，不重启、不改目录、不覆盖 `portico.env`；比对时间用的锚点只在 `$TMPDIR` 且用完即删；`skip` 只在显式接受且没钉修订时出现一行，并写明它只证明了行为 | `tests/e2e/deploy_macos_verify_e2e_test.ts`；`tests/deploy_macos_contract_test.ts` |
...
`deploy/verify.sh` 接着管**已经在跑的那一套**：部署之后先只读地回答「验证的是哪个地址」（`PORTICO_DEPLOY_BIND` 声明，未声明时取监听表里唯一的那个地址；同一端口有两个监听地址就 FAIL 而不猜，绑在所有接口上的入口同样 FAIL），再打三个入口，按名字断言发现页（不是同 `<title>` 的错误壳）、`/public`、`/internal` 的 404、目录信封、Portal 写方法 405、Gateway 不执行工具与审计 403、MCP `initialize`，最后读回每个入口启动时记录的修订（`running-revision`：`run-*.sh` 把 `-e PORTICO_REVISION=<自己检出的 HEAD>` 交给入口，门禁从运行进程的启动环境里取回这一个变量，只取这一个——同一份输出里还有部署导出的其它环境值，环境文件里有凭证，回显就把只读检查变成日志泄漏）、比较每个监听进程的启动时间与它服务的树、以及 `PORTICO_EXPECT_SHA`——进程也按地址匹配，免得多出来的那个监听者替真正的入口回答。启动记录是这里唯一直接等于「重启」的证据：值在 `exec` 时就固定，恢复、`touch` 或时钟都挪不动它，而没被替换的旧进程会如实报出旧修订。修订这条是**必须点名**的：只证明行为，而行为恰好是那个从没被替换的进程照样给出的回答，所以没钉修订就报 `FAIL`（要显式接受才会降级成 `skip`）。之所以要单独一条：`systemctl restart` 的 sudo 被拒时命令非零退出，而旧容器仍在同一批端口上照常服务，于是「端口有回答」看起来和「新修订在服务」一样——上一轮部署就是这么绕过验证的；地址这条同理：内网入口绑在单播地址上，门禁若拿回环默认值去探，会把一份健康的部署报成十条「got HTTP 000」，让人分不清是部署坏了还是门禁问错了地址。macOS 侧的同名门禁在 `deploy/macos/verify.sh`。

`deploy/macos/verify.sh` 管的是**生产那一套**（launchd 单监管四个入口），按「最便宜发现、最贵漏掉」排序回答四件事：检出修订（`PORTICO_EXPECT_SHA` 必须点名，对不上或根本没钉都 FAIL；只有显式接受 `PORTICO_DEPLOY_ALLOW_UNPINNED` 才降级成一行 `skip`）、四个入口是不是由这个检出启动的（`running-revision`：读回 `run.sh` 为每个入口记录的 `PORTICO_REVISION`，与 `$TREE` 的 HEAD 比，读到别的修订或读到没有都 FAIL 并点名）、四个入口各自由哪个进程服务、每个入口是否各自兑现契约。中间那两条是重点，而且是同一个问题的两个读法：`launchctl kickstart -k` 是把 pull 真正送上线的那一步，「pull 了但没重启」会让上一版修订继续在同样的端口上用同样的产品页应答，行为探针看不出区别——因为回答它的正是那个从未被替换的进程。最直接的证据是启动器留下的记录：`run.sh` 导出 `PORTICO_REVISION`（取自 `$ROOT/app` 的 HEAD，写在环境文件之后，`portico.env` 覆盖不了它），值在 `exec` 时固定，只有真正的重启才改得动它；第二个读法才是时间——门禁把每个入口解析到一个 pid（先按声明的回环地址匹配，取不到再退回该端口上的监听者），拿它的启动时刻与 `PORTICO_DEPLOY_TREE/src` 里最新的文件比，进程比它服务的代码旧就 FAIL 并点名进程与文件，因为时间可以被恢复或 `touch` 挪动。门禁只从启动环境里取出 `PORTICO_REVISION` 这一个变量，其余环境值绝不回显。与 Linux 门禁有一处刻意不同：入口没有进程时这里是 FAIL 而不是 `skip`，因为本部署固定是同一个监管者拉起的四个入口，少一个不是未知，是系统没起来；绑定到所有接口这条也不在这里重复——`src/runtime/bind.ts` 本身就拒绝 `0.0.0.0`、`::` 与公网地址，非回环绑定先让入口起不来，门禁再来一遍只会多一条永远为真的断言。四个端口、回环地址与公网 origin 都可从环境覆盖（默认值就是本机部署的形状），这不是给验收开后门：一份只能在宿主机上跑的门禁等于一份没法验证的门禁，只有它能被指向测试自己的监听者，它的失败路径才是被验证过的。
