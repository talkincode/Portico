...
`src/portal/` 是只读 HTTP 入口，消费同一 `CatalogService`。`GET /api/catalog`、`GET /api/catalog/:id`、`GET /api/dashboard`、`GET /api/mcp`、`GET /api/mcp/:id`、`GET /api/web`、`GET /api/web/:id`、`GET /api/cli`、`GET /api/cli/:id`、`GET /api/audit` 与 `GET /`（HTML）对同一身份呈现与 CLI 相同的可见性（审计面仅人类审计者）。发现页对可见记录展示已授权入口：Web 为直连 http(s) 链接，CLI 为转义后的包坐标，其它渠道为转义后的引用，不代理页面。无 actor 头且无会话为匿名；`X-Portico-Actor-*` 对照身份名册解析，不能自封角色；`Authorization: Bearer` / `X-Portico-Session` 解析已登录会话且角色以名册为准。默认绑定 `127.0.0.1`；内网测试允许 RFC1918 单播 `PORTICO_BIND`，拒绝 `0.0.0.0`。POST/PUT/PATCH/DELETE 返回 405，不写目录。登录在 CLI 完成；Portal 只读消费会话文件。

> **需修订：** 原文把 `GET /` 写成治理状态表。现 HTML 是社论杂志风发现壳（PORTICO 顶栏、内容/专题/收藏展示、渠道过滤、明暗 `data-theme`、精选 hero、`/s/:id` 阅读栏），映射的仍是 Agent 表面目录，不是文章 CMS。匿名只能看见 `approved_public`；内部详情对匿名为 HTML 404 且不泄漏入口。收藏为展示项，无写入。
