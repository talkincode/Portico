...
| 部署契约与入口健康门禁 | 中 | ✅ 只读门禁先断言验证的是哪个地址（`PORTICO_DEPLOY_BIND` 声明，未声明时取监听表里唯一的那个地址；同端口有两个就 FAIL 而不猜），且三个入口都不绑在所有接口上；再打三个真实入口：发现页（按发现壳结构判产品页，不只看 200）、`/public` 200、`/internal` 404、目录 `ok:true` 信封、Portal 写方法 405、Gateway 工具调用 POST 405、Gateway 审计匿名 403、MCP `initialize` 返回 `portico`、MCP GET 405、监听进程不比它服务的树更旧、`PORTICO_EXPECT_SHA` 与检出相符时报该条 `ok`；页眉只挂出本部署声明服务的审核入口——声明 `off`（三入口测试部署的默认）时产品页不得出现 `/review` 链接，声明 origin 时链到该 origin | ✅ 入口不在声明的地址上（点名声明的地址与真正应答的地址）、端口无人监听（点名端口）、同一端口两个监听地址（拒绝猜测；声明其中一个也仍 FAIL 并点名另一个）、入口绑在所有接口上、入口停机、错误页冒充产品页（同 `<title>` 同壳）、匿名 `/internal` 返 200、监听进程早于树的写入、`PORTICO_EXPECT_SHA` 与检出不符、**没钉修订且未显式接受**（不设 `PORTICO_DEPLOY_ALLOW_UNPINNED`，或设成 `0`/`no` 一类非真值）——报 `FAIL checkout-revision` 并点名两个变量名、产品页挂出本部署不提供的审核入口（声明 `off` 却出现 `/review`，或声明了 origin 却不链过去）——每条按名字报 `FAIL` 且非零退出 | ✅ 不适用：门禁只用匿名只读探测，不需要身份也不需要 sudo；Portal 与 MCP 仍不持 `--allow-write`，门禁不碰名册 | ✅ 门禁自身无副作用：失败只报名字与非零退出，不重启、不改目录、不留残文件（临时戳只在 `$TMPDIR`）；`skip` 只在显式接受（`PORTICO_DEPLOY_ALLOW_UNPINNED` 取真值）且该条确实不适用时出现，且不冒充通过 | `tests/e2e/deploy_verify_e2e_test.ts`；`tests/deploy_contract_test.ts` |

| macOS 生产部署门禁与运行修订 | 中 | ✅ 只读门禁在宿主机上回答三件事：检出修订（`PORTICO_EXPECT_SHA` 与检出相符时报该条 `ok`）、四个入口（Portal 8788 / Gateway 8789 / MCP 8790 / Review 8791）**各自由哪个进程服务**、每个入口各自兑现契约——Portal `/public` 200、Review `/review/login` 200、公网 origin 上两条对应探测、匿名目录 `ok:true` 信封、匿名 `/internal` 404、Gateway 工具调用 POST 405、MCP `initialize` 返回 `portico`；默认值就是本机部署形状（回环 + 一个公网 origin），四个端口与 origin 都可由环境覆盖，因此门禁本身可以被指向测试自己的监听者并跑出全绿 | ✅ 入口没有进程在服务（点名入口与端口，且是 FAIL 而非 `skip`）、监听进程比 `PORTICO_DEPLOY_TREE/src` 里的代码旧（点名进程与它先于哪个文件写出）、`PORTICO_EXPECT_SHA` 与检出不符、**没钉修订且未显式接受**（不设 `PORTICO_DEPLOY_ALLOW_UNPINNED`，或设成 `0`/`no` 一类非真值）——报 `FAIL checkout-revision` 并点名两个变量名、Gateway 真的执行了工具、MCP 返回 200 但不是 portico 的 initialize 结果、检出里没有 `src` 可比、`mktemp` 不可用——每条按名字报 `FAIL` 且非零退出 | ✅ 不适用：门禁只用匿名只读探测，不需要 sudo，不读名册也不改目录状态 | ✅ 门禁自身无副作用：失败只报名字与非零退出，不重启、不改目录、不覆盖 `portico.env`；比对时间用的锚点只在 `$TMPDIR` 且用完即删；`skip` 只在显式接受且没钉修订时出现一行，并写明它只证明了行为 | `tests/e2e/deploy_macos_verify_e2e_test.ts`；`tests/deploy_macos_contract_test.ts` |
...
`deploy/verify.sh` 接着管**已经在跑的那一套**：部署之后先只读地回答「验证的是哪个地址」（`PORTICO_DEPLOY_BIND` 声明，未声明时取监听表里唯一的那个地址；同一端口有两个监听地址就 FAIL 而不猜，绑在所有接口上的入口同样 FAIL），再打三个入口，按名字断言发现页（不是同 `<title>` 的错误壳）、`/public`、`/internal` 的 404、目录信封、Portal 写方法 405、Gateway 不执行工具与审计 403、MCP `initialize`，最后比较每个监听进程的启动时间与它服务的树、以及 `PORTICO_EXPECT_SHA`——进程也按地址匹配，免得多出来的那个监听者替真正的入口回答。修订这条是**必须点名**的：只证明行为，而行为恰好是那个从没被替换的进程照样给出的回答，所以没钉修订就报 `FAIL`（要显式接受才会降级成 `skip`）。之所以要单独一条：`systemctl restart` 的 sudo 被拒时命令非零退出，而旧容器仍在同一批端口上照常服务，于是「端口有回答」看起来和「新修订在服务」一样——上一轮部署就是这么绕过验证的；地址这条同理：内网入口绑在单播地址上，门禁若拿回环默认值去探，会把一份健康的部署报成十条「got HTTP 000」，让人分不清是部署坏了还是门禁问错了地址。macOS 侧的同名门禁在 `deploy/macos/verify.sh`。

`deploy/macos/verify.sh` 管的是**生产那一套**（launchd 单监管四个入口），按「最便宜发现、最贵漏掉」排序回答三件事：检出修订（`PORTICO_EXPECT_SHA` 必须点名，对不上或根本没钉都 FAIL；只有显式接受 `PORTICO_DEPLOY_ALLOW_UNPINNED` 才降级成一行 `skip`）、四个入口各自由哪个进程服务、每个入口是否各自兑现契约。中间那条是重点：`launchctl kickstart -k` 是把 pull 真正送上线的那一步，「pull 了但没重启」会让上一版修订继续在同样的端口上用同样的产品页应答，行为探针看不出区别——因为回答它的正是那个从未被替换的进程。门禁因此把每个入口解析到一个 pid（先按声明的回环地址匹配，取不到再退回该端口上的监听者），拿它的启动时刻与 `PORTICO_DEPLOY_TREE/src` 里最新的文件比，进程比它服务的代码旧就 FAIL 并点名进程与文件。与 Linux 门禁有一处刻意不同：入口没有进程时这里是 FAIL 而不是 `skip`，因为本部署固定是同一个监管者拉起的四个入口，少一个不是未知，是系统没起来；绑定到所有接口这条也不在这里重复——`src/runtime/bind.ts` 本身就拒绝 `0.0.0.0`、`::` 与公网地址，非回环绑定先让入口起不来，门禁再来一遍只会多一条永远为真的断言。四个端口、回环地址与公网 origin 都可从环境覆盖（默认值就是本机部署的形状），这不是给验收开后门：一份只能在宿主机上跑的门禁等于一份没法验证的门禁，只有它能被指向测试自己的监听者，它的失败路径才是被验证过的。
