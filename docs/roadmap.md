...
| 审计封条校验（篡改可指名） | 高 | ✅ 同一人类审计者经 CLI `audit verify`、Portal `GET /api/audit-verify` 与 MCP `portico_audit_verify` 得到同一份四支柱报告（catalog / identities / gateway / conclusions）：每条已封记录按顺序复算摘要链，`ok:true`、`unsealed:0`，三入口报出同一 tip | ✅ 改动、删除或剪断链上任一记录都会点名该条（seq / kind / id）与原因（`digest` / `missing` / `chain`），不笼统报失败；未封记录如实计入 `unsealed` 而不算已通过；校验本身不得把被改记录改回原状 | ✅ 人类审计者 vs 维护者/只读/匿名（后三者 FORBIDDEN） | ✅ 校验只读：跑过 CLI / Portal / MCP 三入口后四个支柱文件字节不变；把被改字节还原后结论重新 `ok:true` | `tests/audit_seal_test.ts`；`tests/e2e/audit_seal_e2e_test.ts` |
