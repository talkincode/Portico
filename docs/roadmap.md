...
> **需修订（已修订）：** 原文写“当前仓库几乎是空的……没有运行时代码、测试或 CI”。2026-09-13 起仓库已有 Deno 运行时骨架、内部 Registry 目录、Publisher 草稿/内部发布/公开候选、Approval 公开发布审批、Access Control 身份名册、只读 Portal 发现、MCP 渠道登记与授权连接信息，以及 MCP Gateway 鉴权/路由/访问审计（只做门卫，不执行工具、不代理流量）；未实现的模块仍是产品意图，不是现存实现。
...
`src/cli/main.ts`：`identity grant|list|grants` 与 `catalog register|draft|publish|approve|reject|list|get`，一次调用结束，stdout 为 `{ok,data}` / `{ok,error:{code,message}}`。非匿名 catalog 命令对照身份名册解析 `--actor-*`。登录会话与外部 IdP 尚未实现。
...
UI Components、人类安全审计视图、登录会话。标为待核验以外的“已有能力”一律不应被写出。
...
Registry 内部登记、Publisher 草稿/内部发布/公开候选、Approval 通过/拒绝、Access Control 身份名册、Portal 发现/仪表盘、MCP 渠道登记/连接信息、MCP Gateway 鉴权路由，以及 CLI 对应命令已有测试证据。其余一级功能仍为缺口；实现时必须补测并改本表。CLI 行不含登录会话；非匿名 `--actor-*` 与 Portal / Gateway `X-Portico-Actor-*` 必须与身份名册一致。
