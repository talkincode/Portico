...
| 安全审计结论（维护权 ≠ 审计权） | 高 | ✅ 人类审计者 `audit conclude --scope public_boundary --verdict flagged --note ...` 后，`audit conclusions`、Portal `GET /api/conclusions` 与 MCP `portico_conclusions` 看到同一条；审计者读得到自己名下另一作用域的结论（独立性按主体判定，不按结论） | ✅ 维护者/只读者/匿名读写均 `FORBIDDEN`；Agent 声称 auditor 角色被拒（`identity grant` 与结论服务两处）；审计者同时是该主体维护者时 `SELF_AUDIT` 且不落盘；未知主体 `NOT_FOUND`；非法 scope/verdict/未知字段/密钥字段被拒；`flagged` 无 note 被拒；note 含明文密钥（AWS Access Key ID、Google API Key、PEM 私钥头、npm 令牌形值）或控制字符被拒；重复 id `ALREADY_EXISTS`；损坏文件响亮失败 | ✅ 人类审计者 vs 维护者 / 只读者 / 匿名；审计者兼维护者被拒 | ✅ 被拒结论不写结论文件、不改目录、不产生目录变更事件；读不创建文件；复评追加而非覆盖（旧结论仍可读） | `tests/audit_conclusions_test.ts`；`tests/e2e/audit_conclusions_e2e_test.ts` |
