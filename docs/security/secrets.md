...
1. **绝对不存明文**：
   ...
   - 提交的数据如果包含疑似密钥字段（如检测到以 `sk-` 开头、AWS Access Key ID `AKIA`/`ASIA` 形值、Google API Key `AIza` 形值、PEM 私钥头 `-----BEGIN ... PRIVATE KEY-----`，或带有明文密码属性），写入校验逻辑将直接抛出错误并拒绝落盘。入口 URL 还会扫描查询参数名、查询值与 fragment；文档锚点或路径中的普通词（如 `/auth/token#installation`）不是密钥。区域名 `asia-pacific` 不是 Access Key，散文中提到「private key」但不含真实 PEM 头同样不是密钥。
2. **只引用，不托管**：
